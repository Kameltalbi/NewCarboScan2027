-- =====================================================
-- Migration : Réactivation du module Collect
-- =====================================================
-- Cette migration recrée les tables Collect et ajoute le module dans la table modules

-- Recréer les tables Collect (version simplifiée - tables de base uniquement)
...
DROP POLICY IF EXISTS "Users can view their own collect sessions" ON public.collect_sessions;
DROP POLICY IF EXISTS "Users can insert their own collect sessions" ON public.collect_sessions;
DROP POLICY IF EXISTS "Users can update their own collect sessions" ON public.collect_sessions;
DROP POLICY IF EXISTS "Users can delete their own collect sessions" ON public.collect_sessions;

DROP POLICY IF EXISTS "Users can view their collect responses" ON public.collect_responses;
DROP POLICY IF EXISTS "Users can insert their collect responses" ON public.collect_responses;
DROP POLICY IF EXISTS "Users can update their collect responses" ON public.collect_responses;
DROP POLICY IF EXISTS "Users can delete their collect responses" ON public.collect_responses;

DROP POLICY IF EXISTS "Users can view their collect files" ON public.collect_files;
DROP POLICY IF EXISTS "Users can insert their collect files" ON public.collect_files;
DROP POLICY IF EXISTS "Users can delete their collect files" ON public.collect_files;

DROP POLICY IF EXISTS "Users can view their AI suggestions" ON public.collect_ai_suggestions;
DROP POLICY IF EXISTS "Users can update their AI suggestions" ON public.collect_ai_suggestions;
...
-- Trigger pour updated_at (utiliser la fonction existante si elle existe)
...
-- Ajouter le module Collect dans la table modules
INSERT INTO public.modules (slug, name, description, icon, route, category, is_active)
VALUES (
  'collect',
  'CarboScan Collect',
  'Module de collecte de données carbone avec import Excel, OCR et suggestions IA',
  'Database',
  '/app/collect',
  'addon',
  true
...
-- Commentaires
COMMENT ON TABLE public.collect_sessions IS 'Sessions de collecte de données carbone';
COMMENT ON TABLE public.collect_responses IS 'Réponses aux questions de collecte';
COMMENT ON TABLE public.collect_files IS 'Fichiers uploadés pour la collecte (Excel, PDF, etc.)';
COMMENT ON TABLE public.collect_ai_suggestions IS 'Suggestions IA pour les réponses de collecte';
